...
if CLIENT then language.Add( 'object_combine_field_light_small', 'Combine Small Field Light' ) end
...
if SERVER then
	...
	function ENT:StateOn()
		...
		if IsValid( self.Light ) then
			...
			pt:SetKeyValue( 'fov', '120' )
			pt:SetKeyValue( 'spritedisabled', '1' )
			pt:SetKeyValue( 'lightcolor', '127 255 255 250' )
			pt:SetKeyValue( 'NearZ', '15' )
			pt:SetKeyValue( 'FarZ', '250' )
			pt:SetTexture( 'effects/flashlight001' )
		else
...
list.Set( 'SpawnableEntities', 'object_combine_field_light_small', {
	PrintName = '#object_combine_field_light_small',
	ClassName = 'object_combine_field_light_small',
	Category = 'Combine'
} )
